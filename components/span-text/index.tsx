...
export default class BaseSpan {
   ...
   * @returns {boolean}
   */
  static checkPower = (code: string | Array<string>, type: 'contain' | 'must' = 'contain'): boolean => {
    return baseAuthenticated({ code, type, auth: BaseSpan.config.power, passDev: BaseSpan.config.passDev })
  }
  /**
   * 数据权限的判断
   * @param code code  权限code
   ...
   * @returns {boolean}
   */
  static checkPowerData = (code: string | Array<string>, type: 'contain' | 'must' = 'contain'): boolean => {
    return baseAuthenticated({ code, type, auth: BaseSpan.config.powerData, passDev: BaseSpan.config.passDev })
  }
  static SpanText:React.FC<SpanTextProps> = ({ value, children, to, cursor, powerCode, notValueText = '---', style = {}, powerCodeData,
    ...
    if (powerCode) { // 功能权限的判断
      return BaseSpan.checkPower(powerCode) ? normalSpan() : null
    } else if (powerCodeData) { // 数据权限的判断
      return BaseSpan.checkPowerData(powerCodeData) ? normalSpan() : null
    } else {
      return normalSpan()
    }
  }
...
export const SpanText = BaseSpan.SpanText
